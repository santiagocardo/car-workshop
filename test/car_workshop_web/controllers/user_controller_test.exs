defmodule CarWorkshopWeb.UserControllerTest do
  ...
  describe "edit user" do
    ...
    test "renders form for editing chosen user", %{conn: conn, user: user} do
      conn = get(conn, Routes.session_path(conn, :edit, user))
  ...
  describe "update user" do
    ...
    test "redirects when data is valid", %{conn: conn, user: user} do
      conn = put(conn, Routes.session_path(conn, :update, user), user: @update_attrs)
      assert redirected_to(conn) == Routes.session_path(conn, :show, user)

      conn = get(conn, Routes.session_path(conn, :show, user))
      assert html_response(conn, 200) =~ "some updated name"
    end

    test "renders errors when data is invalid", %{conn: conn, user: user} do
      conn = put(conn, Routes.session_path(conn, :update, user), user: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit User"
    end
  end
  ...
  describe "delete user" do
    ...
    test "deletes chosen user", %{conn: conn, user: user} do
      ...
      assert_error_sent 404, fn ->
        get(conn, Routes.session_path(conn, :show, user))
      end
    end
  end
